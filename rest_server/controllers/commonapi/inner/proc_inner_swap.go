...
func SwapWallet(params *context.ReqSwapInfo, innoUID string) *base.BaseResponse {
	...
	if params.TxType == context.EventID_P2C {
		...
		if params.SwapFromPoint.PreviousPointQuantity <= 0 || // 보유 포인트량이 0일경우
			...
			// 전환할 포인트 수량이 없음 에러
			log.Errorf("lack of minimum point quantity [point_id:%v][PointQuantity:%v][absAdjustPointQuantity:%v]", params.SwapFromPoint.PointID, params.SwapFromPoint.PreviousPointQuantity, absAdjustPointQuantity)
